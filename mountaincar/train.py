"""
Train a PPO agent on MountainCarContinuous-v0 with kinetic energy reward shaping.

Usage:
    python train.py
    python train.py --timesteps 200000 --n-envs 8
    python train.py --resume models/ppo_mountaincar_seed42/best_model
"""
...
import config
from wrappers import KineticEnergyShapingWrapper


def make_env(n_envs: int, seed: int, shaped: bool = True):
    def env_fn():
        env = gym.make(config.ENV_ID)
        if shaped:
            env = KineticEnergyShapingWrapper(env, weight=config.SHAPING_WEIGHT)
        return env

    return make_vec_env(env_fn, n_envs=n_envs, seed=seed)
...
def train(args):
    ...
    print(f"\n{'='*50}")
    print(f"  MountainCarContinuous-v0 PPO Training")
    print(f"  run:    {run_name}")
    print(f"  steps:  {args.timesteps:,}")
    print(f"  envs:   {args.n_envs}")
    print(f"  shaping: KE (weight={config.SHAPING_WEIGHT})")
    print(f"{'='*50}\n")

    train_env = make_env(n_envs=args.n_envs, seed=args.seed, shaped=True)
    eval_env = make_env(n_envs=1, seed=args.seed + 1000, shaped=False)  # raw rewards for honest eval

    if args.resume:
        print(f"Resuming from: {args.resume}")
        model = PPO.load(args.resume, env=train_env, device="cpu", tensorboard_log=tb_log)
    else:
        model = PPO(
            env=train_env,
            verbose=1,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO on MountainCarContinuous-v0")
